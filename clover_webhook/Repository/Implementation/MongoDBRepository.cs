...
namespace clover_webhook.Repository.Implementation
{
/// <summary>
    /// MongoDB repository
    /// </summary>
    public partial class MongoDBRepository<T> : clover_webhook.Repository.Services.IRepository<T> where T : BaseEntity
    {
        #region Fields
        ...
        public MongoDBRepository(IOptions<AppSettings> appSettings)
        {
            ...
            ConventionRegistry.Register("SwiftServe Conventions", pack, t => true);

            var client = new MongoClient(appSettings.Value.MongoDbConnectionString);
            var databaseName = new MongoUrl(appSettings.Value.MongoDbConnectionString).DatabaseName;
