...
class PriceService{
    ...
    public async deletePaymentCard(paymentCustomerId:string, cardId: string){
        log("Delete user payment card");
        let cards:any;
        try {
            cards = await stripe.customers.deleteSource(paymentCustomerId,
            cardId);
            return {status:200, message:{data:cards}}
        }catch(err){
            log("Error User delete payment card", err);
            console.log(err)
            return this.stripeErrorHandling(err);
        }
    }
    
    public async createStripeCustomerusingCustomerId(CustomerId:string){
        log("create user payment intent", CustomerId);
        let email = "cus_" + CustomerId+"@swiftserve.com"

        let customer:any;
        try {
            customer = await this.findCustomerUsingStripeEmail(email);
            if(customer === null){
            ...
            log("Error user create stripe customer", err);
            console.log(err)
            return this.stripeErrorHandling(err);
        }
        return customer.id;
    ...
    public async findCustomerUsingStripeEmail(email:string){
        log("verifying customer in stripe", email);
        ...
        let customerData = customer.data.length === 0?null:customer.data[0]
        return customerData;
    }
        
    public async createPaymentIntentusingCustomerIdandAmount(cusotmerStripeId:string, body:any, totalAmount:number){
        log("create payment intent", body);
        let cards:any;
        try {
            let PaymentIntentData:any = {
                amount: Math.round(totalAmount*100),
                currency: 'usd',
                // customer: cusotmerStripeId,
            ...
            if(cusotmerStripeId !== "No Customer"){
                PaymentIntentData["customer"] = cusotmerStripeId;
            }
            
            if(body.CardId){
                if(body.CardId !== null || body.CardId !== "" || body.CardId !== undefined){
                    PaymentIntentData["payment_method"] = body.CardId;
                    PaymentIntentData["confirm"] = true;
                }
            }
            cards = await stripe.paymentIntents.create(PaymentIntentData);
        }catch(err){
            log("Error user create payment intent", err);
            console.log(err)
            return this.stripeErrorHandling(err);
        }
        return cards;
    ...
    public async getCustomerCardsUsingCustomerEmail(stripeCustomerId:string){
        try {
            let cards:any = await stripe.paymentMethods.list({
                customer: stripeCustomerId,
                type: 'card',
              });
            return {status:200, message:{data:cards.data}}
        }catch(err){
            log("Error User get payment card", err);
            console.log(err)
            return this.stripeErrorHandling(err);
        }
        // return cards;
    ...
    public async getAttachmentCardusingPaymentMethod(stripeCustomerId:string, paymentMethodId: string){
        log("get attach payment cards");
        let cards:any;
        try {
            cards = await stripe.paymentMethods.attach(
                paymentMethodId,
                { customer: stripeCustomerId }
              );
              return {status:200, message:{data:cards.data}}
        }catch(err){
            log("Error attach payment card", err);
            console.log(err)
            return this.stripeErrorHandling(err);
        }
    ...
    public async detachmentCardusingPaymentMethod(paymentMethodId: string){
        log("detach payment cards");
        let cards:any;
        try {
            cards = await stripe.paymentMethods.detach(
                paymentMethodId
              );
            return {status:200, message:{data:cards.data}}
        }catch(err){
            log("Error detach card", err);
            console.log(err)
            return this.stripeErrorHandling(err);
        }
    }

    public stripeErrorHandling(err:any){​
            switch (err.type) {​
                case 'StripeCardError':
                    log("StripeCardError", err.message);
                    // throw new DocumentNotFoundError(err.statusCode, this.errorHandlingStripe(err));
                    return {status:err.statusCode, message:{errors:[this.errorHandlingStripe(err)]}}
                    break;
                ...
                case 'StripeAuthenticationError':
                    log("StripeAuthenticationError"+err.statusCode, err.message);
                ...
                default:
                    ...
                    // throw new DocumentNotFoundError(err.statusCode, err.code === undefined?"Something wrong in stripe.":this.errorHandlingStripe(err));
                    return {status:err.statusCode, message:{errors:[this.errorHandlingStripe(err)]}}
                    break;
            }​
    }​
