...
import PickUpFromRestaurant from './PickUpFromRestaurant';
...
export class MenuItems extends Component {
    ...
    handleRadio = (e) => {
        if(e.target.value === 'Pickup'){
            this.setState({menuItems:[{
                Menus:'Pickup',
                timings:localStorage.getItem("storeTimings")
            }]})
        }
    ...
    inventoryList = (tempStoreId) => {
        ...
        axios.get(`/inventory/v1/c/${tempStoreId}/products/?page=1&limit=101`, { baseURL:baseURL, withCredentials:false,headers: { "authorization": token } })
        .then(res => {
            ...
            this.setState({ itemsList: res.data.data, categoryArrayList:[...categoryArrayListData, "Others"], categoryList:[...res.data.productCategories, {_id:"", Name:"Others"}], noCategoryData: result  })
    ...
    render() {
        const restarantData = this.props.location.state ? this.props.location.state.restaurantdetailProps : this.props.location.restaurantdetailProps;
        
        return (
            (restarantData && restarantData.isRestaurantOpen && restarantData.isPickUpEnabled) ?
            <>
                <PickUpFromRestaurant restaurantID={restarantData.restaurantID}
                    restaurantName={restarantData.restaurantName}
                    restaurantImage={restarantData.restaurantImage}
                    restaurantAddress1={restarantData.restaurantAddress1}
                    restaurantAddress2={restarantData.restaurantAddress2}
                    restaurantCity={restarantData.restaurantCity}
                    restaurantState={restarantData.restaurantState}
                    restaurantPin={restarantData.restaurantPin}
                    restaurantTimings={restarantData.restaurantTimings}
                    isRestaurantOpen={restarantData.isRestaurantOpen}
                    isPickUpEnabled={restarantData.isPickUpEnabled}
                    isDeliveryEnabled={restarantData.isDeliveryEnabled} />
                    </>
                :
                <>
                    ...
                    <div className="store-image" style={{ 
                        ...
                        <div className="bg-transition-data">
                            <div className="store-image-content mt-3 ff-poppins" >
                                ...
                                <div className="row bg-none mt-4">
                    ...
                    <div id="menuItems" className="container menuItems" >
                                ...
                                <div className="row">
                                    <div className="col-sm-12">
                                        ...
                                        <div className="headerTabs" style={{border: "1px solid #F2F2F2", display:"inline-block", float: "right", backgroundColor:"none"}}>
                                            ...
                                            {/* <input type="radio" className="btn-check" name="optionsbody" id="option5" autocomplete="off" />
                                            <label className="btn btn-secondary" for="option5">Dine-in</label>
                                            <input type="radio" className="btn-check" name="optionsbody" id="option6" autocomplete="off" />
                                            <label className="btn btn-secondary" for="option6">Pickup</label>
                                            <input type="radio" className="btn-check" name="optionsbody" id="option7" autocomplete="off" />
                                            <label className="btn btn-secondary" for="option7">Delivery</label> */}
                                        </div>
                                        <div className="menuit-name" style={{position:"unset"}}>
                                            {/* <img src={hotel} alt="" />
                                            <h3>{this.state.storeName+"DDDD"}</h3> */}
                                            <UnderLineTabs tabsArray={this.state.menuItems} 
                                            />
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="col-sm-12">
                            <InventoryList itemsList={this.state.itemsList } categoryArrayList={this.state.categoryArrayList} categoryList={this.state.categoryList} props={this.props} state={this.state} navigateToCatergories={this.navigateToCatergories} noCategoryData={this.state.noCategoryData} />
                        </div>
                            {
                            //    this.state.itemsList.map(item => {
                              //      return (
                                //    <div className="col-sm-12 col-md-6 col-lg-3">
                                  //      <div className="">
                                    //        <Link to={{ pathname: "./categories", state:{storeId:this.props.match.params.id, productId:item.id, storeData:this.state.storeName} }} onClick={this.navigateToCatergories} key={item.id} className="categoriesInMenuItems">
                                      //          <div className="inventoryCard justify-content-between">
                                        //            <div className="content show-curson-icon p-2" >
                                          //              <span  className="inventory-title three-dots"  title={item.Name}>{item.Name} </span>
                                            //            <span  className="inventory-content three-dots" title={item.ShortDescription}>{item.ShortDescription} </span>
                                              //          <div className="">
                                                //            {/* <button onClick={this.navigateToCartOrSignIn} className="btn cta-bgbtn"> Add to Bag   <span className="iconimg-wrap"><img src={sbag} alt="" /> </span>  </button> */}
                                                  //          <span  className="pm-2 inventory-price"> $ {item.Price.toFixed(2)} </span>
                                                    //    </div>
                                                //</div>
                                                //<div className="show-curson-icon menuimg-fit">
                                                  //      {item.ProductPictures.length === 0?"":<img src={item.ProductPictures.length === 0?"https://swiftserve-images.s3.us-east-2.amazonaws.com/MicrosoftTeams-image+(8).png":item.PictureUrl+"/"+item.ProductPictures[0]?.PictureId + "_" +item.ProductPictures[0]?.SeoFilename+"."+mime.getExtension(item.ProductPictures[0]?.MimeType)} alt="" />}
                                                    //</div>
                                                //</div>
                                            //</Link>
                                        //</div>
