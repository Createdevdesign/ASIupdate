...
class AwsS3Service implements AwsUploadAssets {
  ...
  public async uploadProductImageInS3Bucket(imageData: any, productId: string ) {
    log(
      "create product image in s3 bucket." + productId
    );
    
    const s3Params = {
      Bucket: bucketName,
      Key: imageData.PictureId +"_"+path.parse(imageData.image.name).name+path.parse(imageData.image.name).ext, // file name you want to save as
  ...
  public async deleteProductImageInS3Bucket(pictureUrl:any){
    log(
